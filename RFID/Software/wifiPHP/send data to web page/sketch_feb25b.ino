...
void get_ip()
{
  ...
  while(1)
  {
    ...
    while(client.available()>0)
    {
      if(client.find("APIP,")) //STAIP APIP..CHECK IT
      {
        delay(1000);
...
void wifi_init()
{
      ...
      if(!No_IP)
      {
        ...
        delay(500);
        connect_wifi("AT+CWJAP=\"Modiano\",\"M@t$ouk@$ajax1996\"",7000);         //provide your WiFi username and password here
...
void Send()
{
      webpage = "<h1>SUNDEDEMENOI AUTH TH STIGMH STO ERGASTHRIO </h1><body bgcolor=f0f0f0>";
      sendwebdata(webpage);
      webpage=name;
      webpage+=dat;
      sendwebdata(webpage);
     ...
     webpage+="\">Click Here for More projects" + String(counter)+ "</a>";
     counter++;
